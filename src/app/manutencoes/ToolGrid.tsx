...
function TypeDot({ type }: { type: string }) {
  const color =
    type === "Preventiva"
      ...
      ? "bg-red-500"
      : "bg-blue-500";
  return <span className={`inline-block w-2 h-2 rounded-full ${color}`} title={type} />;
}

export function ToolGrid({ tools, currentPage, totalPages, total, search }: Props) {
  ...
  function buildPageUrl(page: number) {
    ...
    return `/manutencoes?${params.toString()}`;
  }
      ...
      {tools.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {tools.map((tool) => {
            ...
            const count = tool._count.maintenanceRecords;
            return (
              <Link
                key={tool.id}
              ...
              >
                ...
                <div className="flex items-center gap-2 mt-3">
                  ...
                  >
                    {count} {count === 1 ? "manutenção" : "manutenções"}
                  </span>
                </div>

                {last && (
                  <div className="flex items-center gap-1.5 mt-2 text-xs text-gray-500">
                    <TypeDot type={last.maintenanceType} />
                    <span>
                      {new Date(last.maintenanceDate).toLocaleDateString("pt-BR")}
                    </span>
                    <span className="text-gray-300">·</span>
                    <span>{last.maintenanceType}</span>
                  </div>
                )}

                {!last && (
                  <p className="mt-2 text-xs text-gray-300 italic">Sem manutenções</p>
                )}
              </Link>
            );
          })}
