...
async function seedVolumes() {
  ...
  console.log(`Volumes seed: ${productCount} produtos, ${forecastCount} forecasts.`);
}
...
async function main() {
  ...
  try {
    ...
    console.log(`XLSX seed completed: ${toolCodes.length} tools, ${productCodes.length} products, ${bomCount} BOM items.`);
  } catch {
    console.warn("dadosferramental.xlsx não encontrado — pulando seed de ferramentais/BOM.");
  }
